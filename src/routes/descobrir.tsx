...
function DescobrirPage() {
  ...
  <section className="section-space"><div className="shell"><SectionTitle eyebrow="De bairro em bairro" title="Explore por bairro" /><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{neighborhoods.map((n,i) => <Button key={n} variant={bairro === n ? "default" : "outline"} onClick={() => setBairro(n)} className="h-28 justify-start bg-cover p-5 text-left" style={{ backgroundImage: bairro === n ? undefined : `linear-gradient(oklch(0.2 0.02 52 / 55%), oklch(0.2 0.02 52 / 55%)), url(${businesses[i%businesses.length].image})` }}><span className={bairro === n ? "" : "text-primary-foreground"}>{n}</span></Button>)}</div></div></section></>;
}