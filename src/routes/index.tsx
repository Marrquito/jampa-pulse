...
import { AwardCard, BusinessCard, CategoryCard, CTASection, SectionTitle } from "@/components/platform";
...
function HomePage() {
  return <>
    <section className="relative min-h-[calc(100svh-4rem)] overflow-hidden bg-primary text-primary-foreground">
      ...
      <div className="shell relative flex min-h-[calc(100svh-4rem)] items-center py-16"><div className="max-w-3xl animate-rise"><span className="inline-flex rounded-full border border-primary-foreground/25 bg-primary/35 px-3 py-1.5 text-xs font-bold backdrop-blur">Viva João Pessoa o ano inteiro</span><h1 className="mt-6 text-5xl font-bold leading-[1.02] sm:text-6xl lg:text-7xl">Descubra o que está em alta em João Pessoa.</h1><p className="mt-5 max-w-2xl text-lg text-primary-foreground/80">Restaurantes, bares, lojas, serviços e experiências que estão fazendo sucesso pela cidade.</p><div className="mt-6 flex items-center gap-2 text-sm text-primary-foreground/70"><MapPin className="size-4" /> Feito para descobrir o melhor da cidade</div></div></div>
    </section>
